...
//Si la solicitud se asigno al gotrero debe estar registrada en solicitud como de el.
        ...
        if (tipoNotificacion == tipoNotificacionPreparacion) {
            ...
            data.consultarRes(STORE_LLEGADA_DESPACHO, [idDespacho], function () {
                data.consultarRes(STORE_LISTAR_ID_DESPACHO_CLIENTE, [idDespacho], function (despachos) {
                    ...
                    let valor = '';
                    chatDespacho.enviarChat(req, imei, idDespacho, idClienteRecibe, idClienteEnvia, _CHAT_ENVIA_CAJERO, _CHAT_TIPO_TEXTO, '👋 Gotrero en lugar.', mensaje, valor, idDespachoEstado, function () {
                        return res.status(200).send({ estado: 1, despacho: despachos[0] });
                    }, res);
                }, res);
            }, res);
